...
const reqForRegister = asyncHandler(async (req, res) => {
    const { name, phone, role, dateOfBirth, joiningDate, email, address, password } = req.body
    // check for Empty Values
    if ([name, phone, role, dateOfBirth, joiningDate, email, address, password].some((field) =>
        field?.trim() === '')
    ) {
...
const loginEmp = asyncHandler(async (req, res) => {
    ...
    if (!user) {
        throw new ApiError(404, "User Does not Exist or Not Approved by Admin")
    }
